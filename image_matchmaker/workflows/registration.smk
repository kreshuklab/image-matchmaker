...
rule rigid_alignment:
    ...
    log: f"{log_dir}/image_matchmaker.log"
    shell:
        f"python -m image_matchmaker.registration.rigid_alignment_elastix --fixed_path {{input.fixed_image_n5}} --fixed_key {{params.input_key}} --moving_path {{input.moving_image_n5}} --moving_key {{params.input_key}} --output_dir {log_dir}/{rigid_alignment_dir} --output_key {{params.output_key}};"
...
rule create_mobie_project:
    ...
    log: f"{log_dir}/image_matchmaker.log"
    shell:
        f"python -m image_matchmaker.data_processing.mobie_export --input_path {{input.fixed_image_n5}} --input_key {{params.input_key}} --input_type {fixed_type} {'--semantic_seg' if semantic_seg else ''} --dataset_name {dataset_name} --output_dir {log_dir};"
        ...
        f"python -m image_matchmaker.data_processing.mobie_export --input_path {{input.moving_image_n5}} --input_key {{params.input_key}} --input_type {moving_type} {'--semantic_seg' if semantic_seg else ''} --dataset_name {dataset_name} --output_dir {log_dir};"
...
rule cpd_nonrigid_registration:
    ...
    log: f"{log_dir}/image_matchmaker.log"
    shell:
        f"python -m image_matchmaker.registration.cpd_nonrigid_registration --moving_path {{input.moving_image_n5}} --moving_key {{params.moving_key}} --fixed_path {{input.fixed_image_n5}} --fixed_key {{params.fixed_key}} -o {{params.log_dir}} --w {w} --beta {beta} --lmd {lmd} --maxiter {maxiter};"
...
rule matching:
    ...
    log: f"{log_dir}/image_matchmaker.log"
    shell:
        f"python -m image_matchmaker.registration.match_pointclouds --fixed_pcd {{input.fixed_pcd}} --moving_pcd {{input.moving_pcd}} -o {{params.log_dir}} {min_neighbours_opt} --max_dist {max_dist} --method {matching_method} --tau {sinkhorn_tau} --sinkhorn_max_iter {sinkhorn_max_iter};"
