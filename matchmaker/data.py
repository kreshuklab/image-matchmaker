...
def pcd_np_to_df(pos:np.array, segm_labels=None, reg_gt=None, matching=None):
    ...
    if segm_labels is not None:
        assert len(segm_labels) == N, print(f"Labels of shape {segm_labels.shape} don't correspond to coordinates of shape {pos.shape}") 
        pcd_df["segm_labels"] = segm_labels

    if reg_gt is not None:
        assert len(reg_gt) == N, print(f"Labels of shape {reg_gt.shape} don't correspond to coordinates of shape {pos.shape}") 
