...
def run_registration(fixed_img, moving_img, parameter_map_paths, log_dir, log_name="elastix.log", set_threads=False):
...
def run_pointset_registration(fixed_img, moving_img, parameter_map_paths, fixed_pointset, moving_pointset, log_dir, log_name="elastix.log", set_threads=False):
    logging.info("Start creating parameter object")
    parameter_object = create_parameter_object(parameter_map_paths)
    # Load Elastix Image Filter Object
    elastix_object = itk.ElastixRegistrationMethod.New(fixed_img, moving_img)
...
def serialize_parameter_object(parameter_object, prefix, write_dir):
    ...
    for index in range(parameter_object.GetNumberOfParameterMaps()):
        ...
        parameter_object.WriteParameterFile(parameter_map, write_dir / f"{prefix}.{index}.txt")
